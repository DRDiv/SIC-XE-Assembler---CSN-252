...
int handelBYTE(string x) {
	...
	if (x.at(0) == 'x' || x.at(0) == 'X' || x.at(0) == 'c' || x.at(0) == 'C') {
		if (x.at(1) == '\'' && x.at(x.length() - 1) == '\'') {
			...
			errorMessage += "\nNot Hexadecimal/Character String";
			return 0;
		}
	} else {
		errorMessage += "\nIllegal Operand";
	...
	if (findExpressionType(pl.operand1) != -1) {
		...
		if (!checkLabels(labels, current)) {
			...
			try {
				...
				errorMessage += "\nUnable to Evaluate Given Expression";
				delete e;
				return;
			}
			symInfo* c = new symInfo;
...
void addExtRefs(parsedLine pl, cSect* current) {
	if (current->extRef.size() == 0) {
		...
		errorMessage +=
				"\nMultiple EXTREF in Control Section";
}
...
void addExtDefs(parsedLine pl, cSect* current) {
	if (current->extDef.size() == 0) {
		...
		errorMessage +=
				"\nMultiple EXTDEF in Control Section";
}
...
void getlocCtrIncr(parsedLine pl, cSect* current, bool* started, bool* ended) {
	...
	if (incr == -1) { 
		...
		if (upper.compare("START") == 0) {
			if(pl.lable.length()>6){
				errorMessage += "\nInvalid Control Section Name";
			}
			else if (!*started) {
				*started = true;
		...
		} else if (upper.compare("CSECT") == 0) {
			if(pl.lable.length()>6){
				errorMessage += "\nInvalid Control Section Name";
			}
			else current->name=pl.lable;
		...
		else if (upper.compare("BASE") == 0) {
			current->baseAdd=pl.operand1;
		}
	} else { 
		if (!*started) {
			...
			*started = true;
			current->locCtr = 0;
			if (current->firstInstruction<0)current->firstInstruction=current->locCtr;
			writeFomatted(pl, current);
			addLableToSymTab(pl.lable, current);
